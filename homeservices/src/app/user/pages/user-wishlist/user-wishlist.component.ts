...
export class UserWishlistComponent {
    ...
    this.authService.getAllEmp().subscribe(
      (response) => {
        ...
        this.emp.forEach(employee => {
          this.favoriteStatus.set(employee.employee_id, false);  // Initially set all as not favorite
        });
  ...
  // Filter employees based on toggled favorites
  filteredEmployees(): any[] {
    const query = this.searchQuery.trim().toLowerCase();
    return this.emp.filter(employee => 
      this.favoriteStatus.get(employee.employee_id) === true && // Only show favorited employees
      (!query || 
