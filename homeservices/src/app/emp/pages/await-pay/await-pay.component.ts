...
@Component({
  selector: 'app-await-pay',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './await-pay.component.html',
  styleUrl: './await-pay.component.scss'
})
export class AwaitPayComponent {
  orders: any[] = [];
      ...
      if (this.currentEmployee && (this.currentEmployee.employee_id || this.currentEmployee["employee_id "])) {
        ...
        this.http.post<any[]>('http://localhost/api_homeservices/orders/get_orders_by_employee.php', {
          ...
          this.orders = data.filter(order =>
            order.emp_payment_status === 0 && order.order_status === "successful"
          );
